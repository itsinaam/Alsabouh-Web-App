...
def create_hub(
    ...
    """
    hub = Location(**hub_in.model_dump())
    db.add(hub)
    db.commit()
    db.refresh(hub)
    return hub
...
def list_hubs(
    ...
    if emirate:
        query = query.filter(Location.emirate_jurisdiction.ilike(f"%{emirate.strip()}%"))

    if status_filter:
        query = query.filter(Location.status.ilike(f"%{status_filter.strip()}%"))

    if search:
        term = f"%{search.strip()}%"
        query = query.filter(
            or_(
                Location.hub_name.ilike(term),
                Location.emirate_jurisdiction.ilike(term),
                Location.hub_classification.ilike(term),
                Location.street_address.ilike(term),
                Location.industrial_plot.ilike(term),
                Location.infrastructure_notes.ilike(term),
                Location.direct_contact_phone.ilike(term),
            )
        )

    total = query.count()
    items = query.order_by(Location.id.desc()).offset(skip).limit(limit).all()

    # System-wide KPI metrics for UI cards
    total_registered = db.query(Location).count()
    total_active = db.query(Location).filter(Location.status.ilike("active")).count()
    total_draft = db.query(Location).filter(Location.status.ilike("draft")).count()
